...
$sql = "SELECT MAX(id) as last_invoice_no FROM ingst";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
  $row = $result->fetch_assoc();
  $invoice_no = $row["last_invoice_no"] + 1;
} else {
  $invoice_no = 1;
}
?>
...
<body>
	...
	<h1>INVOICE FORM</h1>
	<div class="container">
		<a href="in.php" class="btn">Include GST</a>
		<a href="in_main.php" class="btn">Exclude GST</a>
	</div>
	<h3 style="text-align: center; color: white;">GST EXCLUDED</h3>
	<form id="invoice-form" action="in_pdf.php" method="POST">
        <input type="hidden" name="pdf-data" id="pdf-data"><br>
		<label for="invoice_no">Invoice Number:</label>
        <input type="text" name="invoice_no" value="<?php echo $invoice_no; ?>" readonly><br><br>
	  ...
	  <label>
        <input type="checkbox" id="same-address" onchange="copyBillingAddress()">
...
<button type="button" id="addTermBtn">Add New Term</button>
 ...
 <div class="product-section">
    <h2>Products</h2>
	<label for="gst" style="display: none;">Include GST:</label>
