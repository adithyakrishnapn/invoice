...
$quotationId = 'ET/ ' . $currentYear . '-' . $nextYear . '/' . 'QT- '.$id;

// Add Sales Quotation or Service Quotation line based on user selection
if ($_POST['quotation_type'] == 'sales') {
...
$pdf->Cell(0, 40, 'QUOTATION', 0, 1,'L');
$pdf->SetFont('Arial', 'B', 13);
$pdf->SetTextColor(0, 0, 255); // blue color
$pdf->Cell(0, 20, 'QT.NO : ' . $quotationId, 0, 0,'L');
$pdf->SetFont('Arial', 'B', 13);
$pdf->Cell(0, 20, 'DATE : '.$current_date, 0, 1,'R');
$pdf->SetFont('Arial', 'B', 14);
$pdf->SetTextColor(255, 0, 0); // set font color to red
$pdf->Cell(0, 5, 'Client-Name & Address', 0, 1);
$pdf->SetFont('Arial', 'B', 12);
$pdf->SetTextColor(0, 0, 255); // blue color
$pdf->Cell(0, 10, $name, 0, 1);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(100, 5, $pdfData['address'], 0, 1);
...
$pdf->Cell(0, 10, $quotationType, 0, 1, 'C');
...
foreach ($pdfData['products'] as $product) {
  ...
  $gstPercentage = 18; // Set the GST percentage here
  $gstAmount = ($pdfData['subTotal'] * $gstPercentage) / 100;
  $totalAmount = $pdfData['subTotal'] + $gstAmount;
  
  $pdf->Cell(100, 10, '(9% CGST + 9% SGST) '.'GST ('.$gstPercentage.'%):', 1, 0, 'R');
  $pdf->Cell(0, 10, 'Rs.'.$gstAmount, 1, 1,'R');
  
  $pdf->SetFont('Arial', 'B', 17);
  $pdf->Cell(100, 10, 'Total:', 1, 0, 'R');
  $pdf->Cell(0, 10, 'Rs.'.$totalAmount, 1, 0, 'R');
